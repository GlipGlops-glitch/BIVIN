    ...
    try:
        ...
        if iframe_element:
            ...
            if iframe:
                ...
                track_selector("get_iframe_by_src", selector, "css", f"src_pattern_{src_pattern}", f"Iframe with src containing {src_pattern}")
                await iframe.wait_for_load_state("domcontentloaded", timeout=30000)
                return iframe
    except Exception as e:
...
async def vintrace_login(page: Page, username: str, password: str, navigate_to_old_url: bool = True):
    ...
    for selector in email_selectors:
        try:
            ...
            track_selector("vintrace_login_email", selector, "css", "email_field", "Email/username input field on login page")
            email_filled = True
            break
        except Exception as e:
    ...
    for selector in password_selectors:
        try:
            ...
            track_selector("vintrace_login_password", selector, "css", "password_field", "Password input field on login page")
    ...
    for selector in login_btn_selectors:
        try:
            ...
            if login_btn:
                ...
                if is_disabled:
                    print(f"  ⚠ Login button is disabled, waiting...")
                    await asyncio.sleep(1)
            ...
            if "ui-icon-reports" in selector:
                ...
                if icon:
                    ...
                    if link:
                        ...
                        await asyncio.sleep(0.3)
                        await link.click()
                        print(f"✓ Clicked Reports menu using icon selector")
                        track_selector("navigate_to_reports_new_ui", selector, "css", "new_ui_reports", "Reports menu via icon in new UI")
                        await wait_for_all_vintrace_loaders(page)
                        await asyncio.sleep(2)
                        return True
            else:
                ...
                if reports_link:
                    ...
                    track_selector("navigate_to_reports_new_ui", selector, "css", "new_ui_reports", "Reports menu item in new UI")
                    await wait_for_all_vintrace_loaders(page)
                    await asyncio.sleep(2)
                    return True
        ...
        for span in spans:
            ...
            if text.strip() == category_name:
                ...
                if parent:
                    ...
                    track_selector("navigate_to_report_category", "span[id$='|Text']", "css", f"category_{category_name}", f"Report category: {category_name}")
                    await wait_for_all_vintrace_loaders(page)
                    await asyncio.sleep(1)
                    return True
    except Exception as e:
        ...
        try:
            ...
            if element:
                ...
                if text.strip() == category_name:
                    ...
                    track_selector("navigate_to_report_category", selector, "css", f"category_{category_name}", f"Report category: {category_name}")
                    await wait_for_all_vintrace_loaders(page)
                    await asyncio.sleep(1)
                    return True
        except Exception as e:
    ...
    try:
        ...
        for row in rows:
            ...
            if text.strip() == category_name:
                ...
                track_selector("navigate_to_report_category", "table row text match", "text", f"category_{category_name}", f"Report category: {category_name}")
                await wait_for_all_vintrace_loaders(page)
                await asyncio.sleep(1)
                return True
    except Exception as e:
        ...
        try:
            ...
            if element:
                ...
                if text.strip() == report_name:
                    ...
                    track_selector("find_and_click_report_by_name", selector, "css", f"report_{report_name}", f"Report: {report_name}")
                    await wait_for_all_vintrace_loaders(page)
                    await asyncio.sleep(1)
                    return True
        except Exception as e:
            ...
            if report_name in text:
                ...
                if run_icon:
                    ...
                    if run_button:
                        ...
                        track_selector("find_and_click_report_by_name", "img[src*='search-grey.png']", "css", f"report_{report_name}_run", f"Run button for report: {report_name}")
                        await wait_for_all_vintrace_loaders(page)
                        await asyncio.sleep(1)
                        return True
    except Exception as e:
    ...
    for selector in close_selectors:
        try:
            ...
            if close_btn:
                ...
                track_selector("close_report_window", selector, "css", "close_report_window", "Close button for report window")
                await wait_for_all_vintrace_loaders(page)
                await asyncio.sleep(1)
                return True
        except Exception as e:
    ...
    for selector in selectors:
        try:
            ...
            if reports_icon:
                ...
                track_selector("navigate_to_reports_old_ui", selector, "css", "old_ui_reports", f"Reports icon in old UI")
                break
        except Exception as e:
...
async def click_vintage_harvest_tab_old_ui(page: Page):
    ...
    for selector in selectors:
        try:
            ...
            if element:
                ...
                track_selector("click_vintage_harvest_tab_old_ui", selector, "css", "vintage_harvest_tab", "Vintage/Harvest tab in old UI")
                await wait_for_all_vintrace_loaders(page)
                await asyncio.sleep(1)
                return True
        except Exception:
    ...
    try:
        ...
        if element:
            ...
            track_selector("click_vintage_harvest_tab_old_ui", "//*[text()='Vintage/Harvest']", "xpath", "vintage_harvest_tab", "Vintage/Harvest tab via xpath")
            await wait_for_all_vintrace_loaders(page)
            await asyncio.sleep(1)
            return True
    except Exception as e:
    ...
    try:
        ...
        for strip in report_strips:
            ...
            if report_title in text_content:
                ...
                track_selector("find_report_strip_by_title", "div.reportStrip", "css", f"strip_{report_title}", f"Report strip for: {report_title}")
                return strip
        ...
        for select in selects:
            ...
            for option in options:
                ...
                if format_type in option_text or option_text == format_type:
                    ...
                    track_selector("select_report_format", "select option", "css", f"format_{format_type}", f"Report format: {format_type}")
                    await asyncio.sleep(0.5)
                    return True
    ...
    try:
        ...
        for label_elem in label_elements:
            ...
            if checkbox_label in label_text:
                ...
                parent = await label_elem.evaluate_handle("el => el.closest('tr') || el.closest('div') || el.closest('table') || el.parentElement")
                ...
                if parent:
                    ...
                    for img in checkbox_imgs:
                        ...
                        if (checked and not is_checked) or (not checked and is_checked):
                            ...
                            track_selector("set_report_checkbox", "img[src*='Checkbox']", "css", f"checkbox_{checkbox_label}", f"Checkbox: {checkbox_label}")
                            await asyncio.sleep(0.3)
                            return True
                        else:
                ...
                if label:
                    ...
                    if checkbox_label in label_text:
                        ...
                        if (checked and not is_checked) or (not checked and is_checked):
                            ...
                            print(f"✓ {'Checked' if checked else 'Unchecked'} '{checkbox_label}'")
                            track_selector("set_report_checkbox", "input[type='checkbox']", "css", f"checkbox_{checkbox_label}", f"Checkbox: {checkbox_label}")
                            await asyncio.sleep(0.3)
                            return True
    ...
    try:
        ...
        for option in options:
            ...
            if text == option_text or option_text in text:
                ...
                track_selector("select_report_dropdown_option", "select option", "css", f"dropdown_{option_text}", f"Dropdown option: {option_text}")
                await asyncio.sleep(0.3)
                return True
...
async def click_generate_button(report_strip):
    ...
    for selector in generate_selectors:
        try:
            ...
            if btn:
                ...
                track_selector("click_generate_button", selector, "css", "generate_button", "Generate button for report")
                await asyncio.sleep(0.5)
                return True
        except Exception:
            continue
    ...
    try:
        btn = await report_strip.query_selector("xpath=.//button[contains(text(), 'Generate')] | .//input[@type='button' and contains(@value, 'Generate')]")
        if btn:
            await btn.scroll_into_view_if_needed()
            await asyncio.sleep(0.3)
            await btn.click()
            print("✓ Clicked 'Generate' button (xpath)")
            track_selector("click_generate_button", ".//button[contains(text(), 'Generate')]", "xpath", "generate_button", "Generate button via xpath")
            await asyncio.sleep(0.5)
            return True
    except Exception as e:
